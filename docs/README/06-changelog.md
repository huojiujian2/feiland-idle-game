...
> 📖 **总览汇总**：[../SUMMARY\_v1.03.md](../SUMMARY_v1.03.md)（2026-08-31 综合修订）
...
> 详见 [`docs/SUMMARY_v1.03.md`](../SUMMARY_v1.03.md) 总览 + [`docs/SECURITY_AUDIT.md`](../SECURITY_AUDIT.md) 完整审计 + [`docs/DEPLOY.md`](../DEPLOY.md) 部署指南。

### 🛡 安全审计与修复（21 个问题）

#### 🔴 P0 致命（5 项已全部修复）

1. **JWT 鉴权（HMAC-SHA256，零依赖）** —— 70+ `/api/player/:u/*` 路由全需要 token

   * `server/middleware/auth.js`：JWT 签发/校验 + `requireAuth/requirePlayerSelf/requireSelfFromBody/requireAdmin`

   * `client/src/api.js`：自动注入 `Authorization: Bearer` 头

   * `client/src/App.vue`：401 全局回调，自动恢复登录态
2. **bcrypt 密码哈希**（`server/middleware/password.js`）—— rounds=10 首选 + pbkdf2 fallback + 老明文自动升级
3. **IP 速率限制**（`server/middleware/rate-limit.js`）—— 零依赖 token bucket，注册 5/min / 登录 10/min（可 env 覆盖）+ 硬上限 10000 + LRU 淘汰
4. **创世预算 NaN 绕过**（`server/engine/genesis.js#forgeEquip`）—— `Number.isFinite()` 校验防 NaN/Infinity 绕过预算
5. **auto-reincarnate 生产环境禁用**（`server/routes/progression.js`）—— `NODE_ENV=production` 时不注册路由

#### 🟡 P1 高级（10 项已全部修复）

1. **PvP requestId 服务端 HMAC 签名**（`server/middleware/nonce.js`）—— 防客户端伪造
2. **灵鸡斗场 createdAt 服务端生成**（`server/routes/cockfight.js`）
3. **web-server.js 反代加 timeout + body 限制**（`server/web-server.js`）—— 上游 30s + body 1MB
4. **express.json body 100KB 限制**（`server/index.js`）
5. **创世装备每日 5% 衰减**（`server/engine/genesis.js#decayEquipsMax`）—— floor 60%
6. **5s 挂机循环改 setTimeout 自递归**（`server/index.js`）
7. **删除重复的 maybeResetWeeklyBossKills timer**（`server/index.js`）
8. **CORS 白名单**（`server/index.js`）—— 默认 `localhost:3000,127.0.0.1:3000`，env 可覆盖
9. **排行榜分页**（`server/routes/leaderboard.js`）—— `?page=N&pageSize=N`（最大 200/页）
10. **PvP/worldboss 时区统一**（多个文件）—— 北京时间

#### 🟢 P2 中级（6 项已全部修复）

1. **审计日志**（`server/middleware/audit-log.js`）—— `res.on('finish')` 记录所有写操作，password/token 自动脱敏
2. **公会公告 text 长度上限**（`server/routes/guild.js`）—— 500 字符
3. **创世 name 字符白名单**（`server/engine/genesis.js`）—— 中文/字母/数字/常见标点
4. **静态资源 ETag + Cache-Control**（`server/index.js`）—— HTML no-cache，资产 1 年 immutable
5. **docker healthcheck 改用** **`/api/diag/health`**（`docker-compose.yml`）
6. **api.js 错误日志打印真实信息**（`client/src/api.js`）—— 5xx 打印 URL+status+message

### 🐛 属性预设 Bug 调查（用户提问触发）

| Bug                                                                       | 修复                                                                     |
| ------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `deleteAttrPresetBySlot` 误删其他预设（`migratePlayer` `filter(Boolean)` 导致索引错位） | `migratePlayer` 不再 filter null 占位 + `deleteAttrPresetBySlot` 直接用原始数组索引 |
| `applyAttrPreset` 余数分配不公平（同权时全给第一个维度）                                     | 余数循环分配给权重最大的前 N 个维度                                                    |

### 🐛 API 负数注入攻击面（用户提问触发）

| 严重度   | Bug                                                        | 修复                                                            |
| ----- | ---------------------------------------------------------- | ------------------------------------------------------------- |
| 🔴 P0 | `saveAttrPreset` delta 负数 → attrPoints 暴涨（50→1049）         | delta 各维度钳到 ≥ 0 + 透支校验                                        |
| 🟡 P1 | `allocateAttributes` 混合负+正 → attributes 变负                 | 单维度分配钳到 ≥ 0                                                   |
| 🟢 P2 | `saveAttrPreset` attributes 不 sanitize（NaN/Infinity/超大数污染） | 入口加 `sanitizeAttrPresetAttributes()`（`MAX_ATTR_VALUE=100000`） |

### 🧠 内存优化（60 用户在线场景）

| 优化                                                            | 收益                            | 文件                                                                                    |
| ------------------------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------- |
| **2 个真泄漏修复**：`meta.arenaBots` 永不清 + `rate-limit` buckets 无上限  | 永久累积从无限 → 硬上限 200 / 10000     | `server/store-sqlite.js` + `server/store-json.js` + `server/middleware/rate-limit.js` |
| **优化 #1**：arenaBots 从 store meta 搬到进程内存 `_arenaBotsCache`     | snapshot 序列化减 500KB+，不参与落盘    | `server/store-sqlite.js` + `server/routes/pvp.js`                                     |
| **优化 #2**：启动参数 `--max-old-space-size=384` + 每分钟 heapPct 监控    | 防 OOM kill，提前告警               | `server/start-all.js` + `server/index.js`                                             |
| **优化 #3**：新增 `/api/diag/health` + `/api/diag/memory` endpoint | 实时监控，docker/k8s/Prometheus 友好 | `server/routes/diag.js`                                                               |

### 🔧 数据迁移安全性（用户提问触发，2 个真bug）

> 用户问"代码更新会否影响用户存档"时主动发现，**不修会导致老用户登录全部崩溃**。
...
| `view.js:140` 直接 `player.logs.slice(-20)` | 老存档缺 `logs` 字段 → TypeError → 所有玩家 view 500                             | 改成 `Array.isArray(player.logs) ? ... : []` |

### 📦 Docker 部署改动

* `Dockerfile`：加内存参数（`NODE_OPTIONS="--max-old-space-size=384"`）+ 默认走 SQLite + healthcheck 改用 `/api/diag/health`

* `docker-compose.yml`：加 `NODE_MAX_OLD_SPACE_SIZE` / `ENABLE_EXPOSE_GC` / `HEAP_WARN_PCT` / `CORS_ORIGIN` / `AUDIT_LOG` / `JWT_SECRET`（强制要求） / `mem_limit`

* 新建 `start-prod.sh`：Linux/Docker 一键启动脚本（自动注入内存参数 + 自动生成 JWT\_SECRET）
...
| 文档                                               | 状态                                        |
| ------------------------------------------------ | ----------------------------------------- |
| [`docs/SUMMARY_v1.03.md`](../SUMMARY_v1.03.md)   | **新增** —— 综合汇总（本批次总览）                     |
| [`docs/SECURITY_AUDIT.md`](../SECURITY_AUDIT.md) | 更新至 12 章（含内存优化 + 修复状态）                    |
| [`docs/DEPLOY.md`](../DEPLOY.md)                 | 新增 —— 部署升级指南（备份/JWT\_SECRET/内存推荐/验证清单/回滚） |
| [`docs/CODE_INDEX.md`](../CODE_INDEX.md)         | 更新至 v4.0 + 3 个新章节（SQLite/JWT/综合修复）        |
| [`docs/AI_NOTES.md`](../AI_NOTES.md)             | 更新（v1.03 时间戳）                             |
